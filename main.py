...
def train():
    ...
    loss = nn.MSELoss()
    model = ST_ResNet(c_conf=c_conf, p_conf=p_conf, t_conf=t_conf, external_dim=external_dim,
                      nb_residual_unit=nb_residual_unit).to(device)
    ...
    for epoch in range(nb_epoch):
        ...
        print("epoch:" + str(epoch) + '\t' + "train_loss:" + str(l_sum / n) + '\t' + "test_loss:" + str(test_loss))
        logger.info("epoch:" + str(epoch) + '\t' + "train_loss:" + str(l_sum / n) + '\t' + "test_loss:" + str(test_loss))
...
def evaluate():
    ...
    best_model = ST_ResNet(c_conf=c_conf, p_conf=p_conf, t_conf=t_conf, external_dim=external_dim,
                           nb_residual_unit=nb_residual_unit).to(device)
    best_model.load_state_dict(torch.load(save_path))

    evaluate_loss = evaluate_model(best_model, loss, test_iter)
    MAE, MAPE, RMSE = evalute_metric(best_model, test_iter, mmn)

    print("evaluate_loss:" + str(evaluate_loss) + '\t' + "MAE:" + str(MAE) + '\t' + "MAPE:" + str(MAPE) + '\t' + "RMSE:" + str(RMSE))
    logger.info("evaluate_loss:" + str(evaluate_loss) + '\t' + "MAE:" + str(MAE) + '\t' + "MAPE:" + str(MAPE) + '\t' + "RMSE:" + str(RMSE))
    # %%
...
if __name__ == '__main__':
    ...
    if 'traintest.h5' in files:
        ...
        X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = load_data(
            ...
            len_test=len_test,
            preprocess_name='cache/preprocessing.pkl', meta_data=True, meteorol_data=True, holiday_data=True)
